...
extern Type* floatType;    // GK1: extern từ file semantics.c
...
ConstantValue* compileConstant(void) {
  ...
  switch (lookAhead->tokenType) {
    ...
    case SB_MINUS:
      eat(SB_MINUS);
      constValue = compileConstant2();
      if( constValue->type == TP_INT){    // GK1: Check them kieu float
...
ConstantValue* compileConstant2(void) {
  ...
  case TK_NUMBER:
    ...
    constValue = makeIntConstant(currentToken->value);
    break;
  case TK_FLOAT:     // Them keyword moi
    eat(TK_FLOAT);
    constValue = makeFloatConstant(currentToken->fvalue);
    break;
  case TK_IDENT:
...
Type* compileType(void) {
  ...
  case KW_STRING:         // GK1: Them 2 keyword moi
    ...
    type = makeStringType();
    break;
  case KW_FLOAT:        // GK1: Them 2 keyword moi
    eat(KW_FLOAT); 
    type = makeFloatType();
    break;
  //
...
Type* compileBasicType(void) {
  ...
  case KW_STRING:         // GK1: make them StringType 
    ...
    type = makeStringType();
    break;
  case KW_FLOAT:         // GK1: make them FloatType 
    eat(KW_FLOAT); 
    type = makeFloatType();
    break;
  default:
...
Type* compileFactor(void) {
  ...
  case TK_STRING:            // GK1: Check token float
    eat(TK_STRING);
    type = stringType;
    break;

  case TK_FLOAT:           // GK1: Check token float
    eat(TK_FLOAT);
    type = floatType;
    break;
  ...
  case TK_IDENT:
    ...
    case OBJ_CONSTANT:
      if(obj->constAttrs->value->type == TP_INT){
        ...
        type = charType;
      } else if(obj->constAttrs->value->type == TP_FLOAT){   //GK1
        type = floatType;
