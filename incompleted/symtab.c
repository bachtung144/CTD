...
Type* floatType;   // GK1: Thêm khởi tạo floatType sử dụng trong parse.c
...
Type* makeFloatType(void) {        // GK1: Tạo hàm makeFloatType
  Type* type = (Type*) malloc(sizeof(Type));
...
int compareTypeInExpression(Type* type1, Type* type2) {
  ...
  if (type1->typeClass == type2->typeClass) {
    ...
    if((type1->typeClass == TP_FLOAT && type2->typeClass == TP_INT) || (type1->typeClass == TP_INT && type2->typeClass == TP_FLOAT)) {
      return 1;
    } else return 0;
  }
...
void freeType(Type* type) {
  switch (type->typeClass) {
    ...
    case TP_FLOAT:         // GK1: Free thêm 2 kiểu dữ liệu mới
      free(type);
      break;
    case TP_ARRAY:
...
// TODO:GK1
ConstantValue* makeFloatConstant(float d) {     // GK1: MakeFloatConstant
  
  ConstantValue* value = (ConstantValue*) malloc(sizeof(ConstantValue));
  value->type = TP_FLOAT;
  value->floatValue = d;
  if(DEBUG == 1) printf("Debug makeFloatConstant: param => %f // constantValue => %f\n", d, value->floatValue);
  return value;
}
...
// TODO:GK1
  ...
  floatType = makeFloatType();      // GK1: khoi tao floatType
}
...
void cleanSymTab(void) {
  ...
  freeType(floatType);         // GK1: Free floatType
}
