...
typedef enum {
  ...
  TK_STRING, TK_FLOAT,     // GK1: Bổ sung để check kí tự mình đang đọc
  
  KW_PROGRAM, KW_CONST, KW_TYPE, KW_VAR,
  KW_STRING, KW_FLOAT,     // GK1: Bổ sung để tạo token cho 2 kiểu dữ liệu mới.
...
typedef struct {
  ...
  union {
    int value;
    float fvalue;      // GK1: Bổ sung để lưu lại giá trị kiểu double
  };
} Token;
